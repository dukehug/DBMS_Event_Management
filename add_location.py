...
with st.form("add_location_form"):
    ...
    with col1:
        ...
        new_localtion_type = st.selectbox("Location Type",["Out door","In door"])
    ...
    if submited_btn:
        if not new_localtion_name or not new_localtion_ct_phone:
            ...
            sql_query= """
                ...
                (loc_name,loc_address,loc_capacity,loc_status,loc_type,loc_contact_person,loc_contact_phone)
                VALUES (?,?,?,?,?,?,?)
            """
            params = (
                new_localtion_name,
                new_localtion_address,
                new_localtion_capacity,
                new_localtion_status,
                new_localtion_type,
                new_localtion_ct_person,
